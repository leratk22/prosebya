...
10. При событии submit (например, transition('submit', { rating, comment })):
   - переход в SUBMITTING
   - вызов submitSurvey({ rating, comment }) из csi-api.js
   - при ok: переход в SUCCESS_4_5 или SUCCESS_1_3 по isHighRating(rating)
   - при error: переход в ERROR с сохранением lastDraft для Retry
</action>
