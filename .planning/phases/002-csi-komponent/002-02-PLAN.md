...
6. Привязка событий: клик баннера → transition('open'); Submit → transition('submit'); Cancel/Close → transition('close'); Retry → transition('retry'); Cancel в error → transition('cancel')
